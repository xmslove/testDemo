...
public class StaffRebateService {
	@Resource
	private IRebateService rebates;
	
	Logger logger = LoggerFactory.getLogger(StaffRebateService.class);
	
	public void rebate(){
		logger.info("销售返利开始");
		//查询销售前一天的22点之后到当天22点之前绑卡的集合
		List<RebateVO> cardList = rebates.findCardList();
		for (RebateVO rebateVO : cardList) {
			String ids = rebateVO.getId();
			String [] ss= ids.split(",");
			List<String> list = Arrays.asList(ss);
			
			int money = list.size()*50*100;  //銷售當天返的錢
			String openid = rebateVO.getOpenid(); //返的人
			...
			//	parm.put("amount", String.valueOf(money)); //转账金额
				...
				parm.put("desc", "aicar銷售返利"); //企业付款描述信息
			...
			if (CollectionUtil.isNotEmpty(restmap) && "SUCCESS".equals(restmap.get("result_code"))) {
				...
				rebates.updateRebateStatus(list);
			}else {
